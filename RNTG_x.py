...
class Noise:
    ...
    def born(self):
        ...
        if self.disp[x][y] < 256:
            self.disp[x][y] += 1
            if self.disp[x][y] > 1:
                if x < self.TILE_WIDTH - self.disp[x][y]:
                    self.disp[x+self.disp[x][y]][y] += self.disp[x][y]
                if x > self.disp[x][y]:
                    self.disp[x-self.disp[x][y]][y] += self.disp[x][y]
                if y < self.TILE_HEIGHT - self.disp[x][y]:
                    self.disp[x][y + self.disp[x][y]] += self.disp[x][y]
                if y > self.disp[x][y]:
                    self.disp[x][y-self.disp[x][y]] += self.disp[x][y]
